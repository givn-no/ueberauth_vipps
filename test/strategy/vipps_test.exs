defmodule Ueberauth.Strategy.VippsTest do
  ...
  setup_with_mocks([
    ...
    csrf_conn = conn(:get, "/auth/vipps", %{}) |> Ueberauth.call(routes)
  ...
  def oauth2_get(%{token: %{access_token: "userinfo_token"}}, "/vipps-userinfo-api/userinfo", _, _),
  ...
  test "handle_request! redirects to appropriate auth uri" do
    conn = conn(:get, "/auth/vipps", %{})
    routes = Ueberauth.init() |> set_options(conn, default_scope: "phoneNumber openid")
    ...
    assert redirect_uri.host == "api.vipps.no"
    assert redirect_uri.path == "/access-management-1.0/access/oauth2/auth"

    assert %{
             "client_id" => "client_id",
             "redirect_uri" => "http://www.example.com/auth/vipps/callback",
             "response_type" => "code",
             "scope" => "phoneNumber openid"
  ...
  test "uid_field is picked according to the specified option", %{csrf_state: csrf_state, csrf_conn: csrf_conn} do
    conn = conn(:get, "/auth/vipps/callback", %{code: "uid_code", state: csrf_state}) |> set_csrf_cookies(csrf_conn)
  ...
  test "userinfo is fetched according to userinfo_endpoint", %{csrf_state: csrf_state, csrf_conn: csrf_conn} do
    conn =
      conn(:get, "/auth/vipps/callback", %{code: "userinfo_code", state: csrf_state}) |> set_csrf_cookies(csrf_conn)
